...
export async function verifyEmailController(req, res) {
    try {
        ...
        // Sequelize: findByPk (find by primary key - UUID)
...
export async function uploadAvatar(req, res) {
    try {
        ...
        await user.update({ avatar: upload.data.url })
...
export async function updateUserDetails(req, res) {
    try {
        ...
        if (password) updateData.password = hashPassword
...
export async function verifyPassword(req, res) {
    try {
        ...
        // Sequelize: findByPk with password field
        const user = await User.findByPk(userId, {
        ...
        return res.json({
            ...
            success: true,
            email: user.email,
            userId: user.id // UUID instead of _id
        });
...
export async function userDetails(req, res) {
    try {
        ...
        // Sequelize: findByPk with attributes exclusion
        const user = await User.findByPk(userId, {
...
export async function googleLoginController(req, res) {
    try {
        ...
        // Sequelize: Tìm user theo googleId trước, sau đó theo email
        ...
        if (!user) {
            ...
            if (user) {
                ...
                if (!user.avatar) updateData.avatar = picture || "";
        ...
        const accessTokenJWT = await generatedAccessToken(user.id);
        const refreshToken = await generatedRefreshToken(user.id);
...
export async function getCustomerAnalytics(req, res) {
    try {
        ...
        if (startDate || endDate) {
            ...
            if (startDate) orderQuery.createdAt[Op.gte] = new Date(startDate);
            if (endDate)   orderQuery.createdAt[Op.lte] = new Date(endDate);
        }
            ...
            if (order.customerId) {
                ...
                if (!customerStats[custKey]) {
                    customerStats[custKey] = {
                        ...
                        orderCount: 0,
                        totalRevenue: 0,
                        joinedDate: order.customer?.createdAt || order.createdAt,
                    };
                }
